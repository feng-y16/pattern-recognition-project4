...
label_pred = KMeans(n_clusters=3, random_state=9).fit_predict(data)#调用C means算法
#######################################################################################################
for i in range(0,len(data)):#作图
    if label_pred[i]==0:
        ...
        plt.scatter(data[i, 0], data[i, 1],1.8, c='y',label='cluster 3')
plt.xlabel('Extracted feature 1')
plt.ylabel('Extracted feature 2')
plt.title("Unnormalized C means (3 clusters)")
labelpool=['cluster 1','cluster 2','cluster 3']
...
plt.savefig("C_means_withoutnorm_3clusters.png")
plt.show()